...
1. Before migration, cordon the nodes in all zones, with the exception of the zone where you want the volumes to be located. To cordon the nodes, run the following command:
   ...
   ```
2. Execute the Automation Suite on Linux to Automation Suite on EKS/AKS migration by running the command in Step 1 of [Execution](https://docs.uipath.com/automation-suite/automation-suite/2023.10/installation-guide-eks-aks/migrating-from-automation-suite-on-linux-to-automation-suite-on-eksaks#execution).
