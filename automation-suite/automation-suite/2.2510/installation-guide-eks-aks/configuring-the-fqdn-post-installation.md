...
:::note
Changing the FQDN requires new server certificates. If a new certificate is not available, you have two options: either continue with the new self-signed certificate configured by the installer automatically, or stop the installation and bring in a new CA-issued certificate. You can configure the certificate via the `server_certificate` field in the `input.json` file. For an Automation Suite installation in a shared cluster, the installer can configure a new certificate automatically only if it has admin privileges in the `&lt;istio-system&gt;` namespace.
:::

If the Automation Suite installer is not granted admin privileges, you must follow the instructions in the note at the end of [Configuring Istio and installing the WASM plugin for routing](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide-eks-aks/installing-and-configuring-the-service-mesh#step-2%3A-configuring-istio-and-installing-the-wasm-plugin-for-routing) before proceeding with the following steps.
