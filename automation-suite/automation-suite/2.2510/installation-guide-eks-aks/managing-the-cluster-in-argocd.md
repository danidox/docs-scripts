...
* **SSO** – recommended authentication method. You can enable SSO authentication post-installation. For instructions, see [Enabling SSO for ArgoCD](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide-eks-aks/managing-the-cluster-in-argocd#enabling-sso-for-argocd).
...
For details on the ArgoCD account types and their permissions, see [Managing the cluster in ArgoCD](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide-eks-aks/managing-the-cluster-in-argocd#managing-the-cluster-in-argocd).
...
1. Add the following lines to the `input.json` file:
   ...
   ```
2. To allow the Automation Suite installer to generate the configuration, run the following command.
   ```
