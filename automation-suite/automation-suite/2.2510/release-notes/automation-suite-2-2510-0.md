...
title: "2.2510.0"
visible: true
slug: "automation-suite-2-2510-0"
---
...
* [Linux installation guide](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide/automation-suite-overview)
...
If you plan to enable these new products, make sure to check out the [cross-product dependencies](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide/automation-suite-products) and that you meet all the [prerequisites](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide/hardware-and-software-requirements#prerequisites-at-a-glance).

All the details about the installation and configuration of these newly onboarded products are available in the [Automation Suite on Linux Installation Guide](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide/preparing-the-installation). For details on how make the most out of the functionality of these products, refer to the individual product guides:

* [Integration Service for Automation Suite User Guide](https://docs.uipath.com/integration-service/automation-suite/2.2510/user-guide/introduction)
...
Single-node production deployment is supported only when all required conditions are met. These conditions are essential to maintain stability and reliability in production. For details, refer to [Single-node production deployment](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide/deployment-architecture#single-node-production-deployment).
...
For details, refer to [Configuring the L7 Load Balancer (Optional)](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide/configuring-the-l7-load-balancer-optional).
...
For configuration details, refer to the [Network requirements](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide/network-requirements) section.
...
For details, refer to [Backing up and restoring the cluster](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide/backing-up-and-restoring-the-cluster).
...
:::important
Using Microsoft SQL Server for the `AutomationSuite_Airflow` database is no longer supported.
:::

Refer to [SQL Requirements for Process Mining](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide/configuring-sql-databases#sql-requirements-for-process-mining) and [Database configuration](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide/database-configuration#bring-your-own-databases) for more information on how to set up and configure the PostgreSQL `AutomationSuite_Airflow` metadatabase.
...
We have expanded our OS support to include RHEL 9.6. For details on the supported RHEL versions, see [Compatibility Matrix](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide/hardware-and-software-requirements#rhel-compatibily-matrix).
...
For details, refer to [Optional: Configuring the monitoring solution](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide/monitoring-configuration#optional%3A-configuring-the-monitoring-solution) and [Monitoring Automation Suite](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide/monitoring-automation-suite).
...
For details, refer to the [Migration and upgrade](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide/migrating-standalone-products-to-automation-suite) section.
...
As an alternative, use [Automation Suite on EKS/AKS](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide-eks-aks/automation-suite-overview), or contact UiPath Support for assistance.
...
We fixed the issue in [Automation Suite 2.2510.1](https://docs.uipath.com/automation-suite/automation-suite/2.2510/release-notes/automation-suite-2-2510-1#bug-fixes).

### GPU enablement fails in offline clusters with in-cluster registry

**Erratum - added December 19, 2025:** An issue prevents the`uipathctl rke2 gpu --enable` command from deploying the `nvidia-gpu-plugin` DaemonSet pods on GPU nodes. The issue occurs in offline clusters that use an in-cluster registry.

To address the issue, you must update the GPU plugin manifest before running the `uipathctl rke2 gpu --enable` command. Replace `<Installer_Directory>` with the path to your installer directory (such as `/opt/UiPathAutomationSuite`), and run the following command:

```
sed -i "s/REGISTRY_PLACEHOLDER/localhost:30071/g" "<Installer_Directory>/Infra_Installer/gpu_plugin/nvidia-device-plugin.yaml"
```

We fixed the issue in [Automation Suite 2.2510.1](https://docs.uipath.com/automation-suite/automation-suite/2.2510/release-notes/automation-suite-2-2510-1#bug-fixes).

### Upgrade fails due to http-echo image seeding error

**Erratum - added December 19, 2025:** An issue prevents the Automation Suite upgrade from completing due to a failure in seeding the `hashicorp/http-echo:1.0` image.

To address the issue, you must manually remove the `http-echo` image reference from the `.images` section in `versions/helm-charts.json` and `versions/1.34.1/helm-charts.json`, and then perform the registry-specific operations again.

We fixed the issue in [Automation Suite 2.2510.1](https://docs.uipath.com/automation-suite/automation-suite/2.2510/release-notes/automation-suite-2-2510-1#bug-fixes).

### Hostname case sensitivity causes upgrade failure

**Erratum - added December 19, 2025:** An issue prevents upgrade operations from completing when a node hostname is uppercase. The issue is due to Kubernetes normalizing the `kubernetes.io/hostname` label to lowercase, which can cause a mismatch during upgrade-server deployment creation.

To address the issue, refer to the [Troubleshooting](https://docs.uipath.com/automation-suite/automation-suite/2.2510/installation-guide/upgrade-failure-due-to-uppercase-hostname) section.

We fixed the issue in [Automation Suite 2.2510.1](https://docs.uipath.com/automation-suite/automation-suite/2.2510/release-notes/automation-suite-2-2510-1#bug-fixes).

### Kerberos authentication limitations

**Erratum - added December 19, 2025:** Kerberos-based authentication is currently not supported for Integration Service.
...
| AI Computer Vision | 2.2510.0 | ✅ | [AI Computer Vision release notes](https://docs.uipath.com/ai-computer-vision/automation-suite/2.2510/user-guide/release-notes-2-2510-0) |
...
| Document Understanding | 2.2510.0 | ✅ | [Document Understanding AI Center-based projects release notes](https://docs.uipath.com/document-understanding/automation-suite/2.2510/release-notes/release-notes-22510)  [Document Understanding modern projects release notes](https://docs.uipath.com/document-understanding/automation-suite/2.2510/release-notes/release-notes-22510) |
