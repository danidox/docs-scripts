...
# Global Playwright browser instance for reuse (much faster than creating new browser per URL)
GLOBAL_PLAYWRIGHT_MANAGER = None
...
class PlaywrightBrowserManager:
    """Manages a single Playwright browser instance for reuse across multiple URLs."""
    def __init__(self):
    ...
    def get_browser(self):
        ...
        try:
            from playwright.sync_api import sync_playwright
            # Start the playwright process (keeps it alive)
            pw = sync_playwright()
            self.playwright = pw.start()
            self.browser = self.playwright.chromium.launch(headless=True)
            print("🚀 Playwright browser initialized (will be reused for all URLs)")
            return self.browser
        except ImportError:
            return None
        except Exception as e:
            print(f"⚠️  Failed to initialize Playwright browser: {e}")
            return None
    
    def cleanup(self):
        """Clean up the browser and playwright instances."""
        if self.browser is not None:
            try:
                self.browser.close()
            except Exception:
...
def _init_playwright_browser():
    """Initialize global Playwright browser instance (reused for all URLs - much faster)."""
    global GLOBAL_PLAYWRIGHT_MANAGER
    if GLOBAL_PLAYWRIGHT_MANAGER is None:
        GLOBAL_PLAYWRIGHT_MANAGER = PlaywrightBrowserManager()
    return GLOBAL_PLAYWRIGHT_MANAGER.get_browser()

def _cleanup_playwright_browser():
    """Clean up global Playwright browser instance."""
    global GLOBAL_PLAYWRIGHT_MANAGER
    if GLOBAL_PLAYWRIGHT_MANAGER is not None:
        GLOBAL_PLAYWRIGHT_MANAGER.cleanup()
        GLOBAL_PLAYWRIGHT_MANAGER = None

def safe_get_html_with_playwright(url: str, session: requests.Session, timeout: int = 30000) -> requests.Response:
    """
    Fetches URL using Playwright headless browser, waiting for page to fully load
    and DocContainer to appear. Reuses a global browser instance for speed.
    Falls back to regular requests if Playwright unavailable.
    Returns a requests.Response-like object with .text, .content, .url, .status_code.
    """
    # Get or create browser instance
    browser = _init_playwright_browser()
    
    # If Playwright still not available, fall back
    if browser is None:
        return safe_get_html(url, session=session)
    
    try:
        # Create a new context for this URL (lightweight, reuses browser)
        context = browser.new_context()
        page = context.new_page()
        
        # Copy cookies from requests session to Playwright
        ...
        # Wait for DocContainer to appear (this also waits for JS to execute)
        try:
            page.wait_for_selector("#DocContainer", timeout=15000, state="attached")
            print(f"   ✅ DocContainer found, page loaded")
            # Small additional wait for any remaining async content
            page.wait_for_timeout(500)  # Reduced from 1000ms to 500ms
        except Exception:
            print(f"   ⚠️  DocContainer not found, but continuing with page content")
            # Still wait a bit for any async content
            page.wait_for_timeout(1000)
        
        # Get the final HTML content
        html_content = page.content()
        final_url = page.url
        status_code = 200  # Playwright doesn't expose status easily, assume 200 if we got content
        
        # Close the page and context (lightweight cleanup, browser stays open)
        try:
            page.close()
            context.close()
        except Exception:
        ...
        return MockResponse(html_content, final_url, status_code)
            
    except Exception as e:
        print(f"   ⚠️  Playwright fetch failed: {e}, falling back to regular fetch")
        return safe_get_html(url, session=session)
...
def html_to_markdown_github(url, position_counter=None, crawl=True):
    ...
    try:
        ...
        while queue:
            ...
            try:
                # Use Playwright to fetch page and wait for content to load (handles JS-rendered content)
                response = safe_get_html_with_playwright(current_url, session=session)
                final_url = response.url
                soup = BeautifulSoup(response.content, "html.parser")

                # Debug: Check what we got
                title_tag = soup.find("title")
                print(f"\n🔍 Debugging page: {current_url}")
                if final_url != current_url:
                    print(f"   ↪️  Redirected to: {final_url}")
                print(f"   Status: {response.status_code}")
                print(f"   Title: {title_tag.string if title_tag else 'No title found'}")
                
                # Check for DocContainer
                doc_container = soup.find("div", id="DocContainer")
                if not doc_container:
                    # Try alternative containers
                    alt_containers = soup.find_all("div", class_=re.compile("doc|content|main", re.I))
                    print(f"   ⚠️  DocContainer not found. Found {len(alt_containers)} alternative content divs")
                    if alt_containers:
                        print(f"   Alternative divs found with classes: {[div.get('class') for div in alt_containers[:3]]}")
                    # Check if it's a redirect or error page
                    if "404" in (title_tag.string if title_tag else "").lower() or "not found" in (title_tag.string if title_tag else "").lower():
                        print(f"   ❌ This appears to be a 404/error page")
                    # Check for redirects
                    meta_refresh = soup.find("meta", attrs={"http-equiv": re.compile("refresh", re.I)})
                    if meta_refresh:
                        print(f"   ↪️  Page has meta refresh (redirect): {meta_refresh.get('content', '')}")
                ...
                if doc_container:
                    ...
                    # NEW: escape only angle brackets in plain text (not in code/HTML)
                    ...
                    # NEW: ensure any <img> that leaked into Markdown table rows becomes ![alt](src)
                    ...
                    # NEW: drop spaces before nested sublists and between sibling items
                    ...
                    # NEW: remove <...> around bare http/https links
...
def pick_sidebar_root(soup: BeautifulSoup) -> Optional[Tag]:
    ...
    if not uls:
        print(f"   ⚠️  No <ul> elements found in page")
        # Debug: show what navigation-like elements exist
        nav_elements = soup.find_all(["nav", "aside", "div"], class_=re.compile("nav|menu|sidebar", re.I))
        if nav_elements:
            print(f"   Found {len(nav_elements)} navigation-like elements: {[el.name for el in nav_elements[:5]]}")
        return None
    # Debug: show info about found uls
    print(f"   Found {len(uls)} <ul> elements (largest has {max(len(ul.find_all('li')) for ul in uls)} items)")
...
def build_sidebar_from_url(url: str) -> Dict:
    ...
    if GLOBAL_SESSION is None:
        GLOBAL_SESSION = requests.Session()
    # Use Playwright to fetch page and wait for content to load
    resp = safe_get_html_with_playwright(url, session=GLOBAL_SESSION, timeout=30000)
    ...
    for tname in list(BANNED_TAGS):
        for t in soup.find_all(tname):
            t.decompose()
    print(f"\n🔍 Looking for sidebar in: {url}")
    root = pick_sidebar_root(soup)
    if not root:
        raise RuntimeError(f"Could not locate a sidebar <ul> in this page. URL: {url}")
...
def write_sidebar_and_metadata(url: str) -> None:
    ...
    print(f"⏱️  Sidebar/metadata generation time: {elapsed_time:.2f}s")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--url",
        type=str,
        help="URL to convert (if not provided, will prompt for input).",
    )
    ...
    try:
        while True:
            if args.url:
                target_url = args.url
                args.url = None  # Clear it so we don't reuse it in the loop
                should_exit_after = True  # Exit after processing when URL is provided via CLI
            else:
                target_url = input("Enter the URL of the starting HTML page (or press Enter to quit): ").strip()
                if not target_url:
                    print("No URL entered. Exiting.")
                    break
                should_exit_after = False  # Continue loop for interactive mode
            ...
            print(f"⏱️  Total conversion time: {conversion_time:.2f}s")
            ...
            try:
                ...
                sidebar_time = time.time() - sidebar_start_time
                print(f"❌ Sidebar/metadata generation failed: {e}")
                print(f"⏱️  Sidebar/metadata attempt time: {sidebar_time:.2f}s")

            overall_time = time.time() - overall_start_time
            print(f"⏱️  Total processing time: {overall_time:.2f}s")
            print("-" * 72)
            
            # Exit after processing if URL was provided via CLI
            if should_exit_after:
                break
    finally:
        # Clean up Playwright browser when done
        _cleanup_playwright_browser()
