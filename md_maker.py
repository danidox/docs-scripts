...
DOCS_PROTECTED_HOSTS = {"docs-dev.uipath.com", "docs-staging.uipath.com", "preview-docs.uipath.com"}  # protected environments
DOCS_PUBLIC_HOSTS = {"docs.uipath.com"}  # public docs
...
AUTH_STORAGE_STATE_LOADER: Callable[[], Optional[dict]] | None = None
AUTH_SESSION_CONFIGURER: Callable[[requests.Session, str], None] | None = None
AUTH_LOGIN_HANDLER: Callable[[str, requests.Session], bool] | None = None
AUTH_PROTECTED_URL_MATCHER: Callable[[str], bool] | None = None


def set_auth_hooks(
    *,
    storage_state_loader: Callable[[], Optional[dict]] | None = None,
    session_configurer: Callable[[requests.Session, str], None] | None = None,
    login_handler: Callable[[str, requests.Session], bool] | None = None,
    protected_url_matcher: Callable[[str], bool] | None = None,
) -> None:
    """Install optional auth hooks for environment-specific wrappers such as md_maker_dev.py."""
    global AUTH_STORAGE_STATE_LOADER, AUTH_SESSION_CONFIGURER, AUTH_LOGIN_HANDLER, AUTH_PROTECTED_URL_MATCHER
    AUTH_STORAGE_STATE_LOADER = storage_state_loader
    AUTH_SESSION_CONFIGURER = session_configurer
    AUTH_LOGIN_HANDLER = login_handler
    AUTH_PROTECTED_URL_MATCHER = protected_url_matcher


def configure_console_utf8() -> None:
    """Force UTF-8 console I/O on Windows so emoji log lines render correctly."""
    if os.name != "nt":
        return
...
def _auth_requires_login(url: str) -> bool:
    if AUTH_PROTECTED_URL_MATCHER is not None:
        try:
            return AUTH_PROTECTED_URL_MATCHER(url)
        except Exception:
            return False
    return False


def _load_auth_storage_state() -> Optional[dict]:
    if AUTH_STORAGE_STATE_LOADER is None:
        return None
    try:
        return AUTH_STORAGE_STATE_LOADER()
    except Exception:
        return None
...
def _requests_session_to_playwright_cookies(session: requests.Session | None) -> List[dict]:
...
def _mojibake_score(text: str) -> int:
    return text.count("\ufffd") * 4 + sum(text.count(marker) for marker in MOJIBAKE_MARKERS)


def fix_mojibake_text(text: str) -> str:
    """
    Repair common UTF-8-as-CP1252 mojibake sequences such as:
      "âœ…" -> "✅"
      "â€™" -> "'"
      stray "Â" -> <removed>
    """
    ...
    best = best.replace("\u00c2\u00a0", "\u00a0")
    best = re.sub(r"(?<=\S)\u00c2(?=\s)", "", best)

    return best


def normalize_response_encoding(resp: requests.Response) -> requests.Response:
    """
    Force a sane text encoding for HTML responses before anything reads resp.text.
    Docs pages are UTF-8 in practice; requests sometimes guesses Latin-1/CP1252,
    which produces mojibake in the generated Markdown.
    """
    ...
    candidates: list[str] = []
    for encoding in (resp.encoding, resp.apparent_encoding, "utf-8", "cp1252"):
        if encoding and encoding not in candidates:
            candidates.append(encoding)

    best_encoding = resp.encoding or "utf-8"
    ...
    for encoding in candidates:
        try:
            ...
            continue
        repaired = fix_mojibake_text(decoded)
        score = _mojibake_score(repaired)
        if best_score is None or score < best_score:
            best_encoding = encoding
            best_score = score

    resp.encoding = best_encoding
    return resp
    ...
    while True:
        ...
        try:
            ...
            if 500 <= resp.status_code < 600:
                ...
                continue
            resp.raise_for_status()
            normalize_response_encoding(resp)
...
def _get_sidebar_html_with_playwright(url: str, session: requests.Session | None = None) -> str | None:
    ...
    try:
        ...
        if PLAYWRIGHT_DRIVER is None:
            ...
            storage_state = _load_auth_storage_state()
...
def safe_get_html(url: str, session: requests.Session, **kw) -> requests.Response:
    """
    GET wrapper:
      - Uses the shared retry flow for all hosts.
      - Optional auth hooks can retry login-wall pages for environment-specific wrappers.
    """
    ...
    if looks_html and _auth_requires_login(url) and looks_like_login_wall(resp.text):
        if AUTH_SESSION_CONFIGURER is not None:
            try:
                AUTH_SESSION_CONFIGURER(session, url)
                resp = sso_get_with_retries(url, session=session, **kw)
            except Exception:
                pass

        ct2 = resp.headers.get("Content-Type", "")
        if ("html" in ct2 or ct2 == "") and looks_like_login_wall(resp.text) and AUTH_LOGIN_HANDLER is not None:
            try:
                if AUTH_LOGIN_HANDLER(url, session):
                    resp = sso_get_with_retries(url, session=session, **kw)
            except Exception:
                pass
...
def transform_availability_images(container: BeautifulSoup) -> None:
    """Replace inline <img> with availability emoji."""
...
AVAILABILITY_IMAGE_SRC_TO_EMOJI = {
    "https://docs-dev.uipath.com/api/binary/test-cloud/2/636657/359336": "❌",
    "https://docs-dev.uipath.com/api/binary/test-cloud/2/636657/359344": "✅",
}


def transform_availability_images(container: BeautifulSoup) -> None:
    """Replace known availability/status images with emoji."""
    for img in list(container.find_all("img")):
        alt = (img.get("alt") or "").strip().lower()
        src = (img.get("src") or "").strip()
        if src in AVAILABILITY_IMAGE_SRC_TO_EMOJI:
            img.replace_with(NavigableString(AVAILABILITY_IMAGE_SRC_TO_EMOJI[src]))
        elif alt in ("available", "yes"):
            img.replace_with(NavigableString("✅"))
        elif alt in ("not available", "no"):
            img.replace_with(NavigableString("❌"))
...
def transform_admonitions(container: BeautifulSoup) -> None:
    ...
    for div in container.find_all("div", class_=True):
        ...
        if div.find_parent("li") is not None:
            ...
            # Mark top-level notes so indent_notes_like_list_content
            # won't pull them into a preceding list item.
            admonition_block = f"\nTOPLEVELNOTEMARKER\n{block}"
...
def indent_images_like_list_content(md_text: str) -> str:
    ...
    while i < n:
        ...
        while i < n:
            ...
            # Break on ANY list item (sibling or nested) so the outer
            # loop processes it with the correct cont_indent for images.
            if list_head_re.match(nxt):
                break

            if not nxt.strip():
                # Look ahead: if next non-blank line is an image, skip blanks
                peek = i + 1
                while peek < n and not lines[peek].strip():
                    peek += 1
                if peek < n and re.match(r'^\s*!\[', lines[peek]):
                    out.append(nxt)
                    i += 1
                    continue
                out.append(nxt)
            ...
            # Re-indent images at any indent level to cont_indent
            if re.match(r'^\s*!\[', nxt):
                out.append(cont_indent + nxt.lstrip(' '))
                i += 1
                continue
...
def indent_notes_like_list_content(md_text: str) -> str:
    ...
    fence_close_re = re.compile(r'^\s*:::\s*$')
    code_fence_re = re.compile(r'^\s*(```|~~~)')
    toplevel_re = re.compile(r'^\s*TOPLEVELNOTEMARKER\s*$')

    inside_code = False

    def _is_toplevel_fence(idx: int) -> bool:
        """Check if a fence at idx is preceded by a TOPLEVEL_NOTE marker."""
        k = idx - 1
        while k >= 0 and not lines[k].strip():
            k -= 1
        return k >= 0 and toplevel_re.match(lines[k])
    ...
    while i < n:
        ...
        # Skip blank line + optional TOPLEVEL_NOTE marker before fence check
        if i < n and not lines[i].strip():
            if i + 1 < n and fence_open_re.match(lines[i + 1]) and lines[i + 1].startswith(base_indent):
                if not _is_toplevel_fence(i + 1):
                    i += 1

        if i < n and fence_open_re.match(lines[i]) and lines[i].startswith(base_indent) and not _is_toplevel_fence(i):
            block: List[str] = []
            while i < n:
                cur = lines[i]
                block.append(cur)
        ...
        while i < n:
            nxt = lines[i]
            # Break on ANY list item (sibling or nested) so the outer
            # loop processes it with the correct cont_indent for notes.
            if list_head_re.match(nxt):
                break
            if code_fence_re.match(nxt):
                break
            # Re-indent ::: fences at base_indent level into the list item
            # (but not top-level notes that are siblings of the list)
            if fence_open_re.match(nxt) and nxt.startswith(base_indent) and len(nxt) - len(nxt.lstrip(' ')) == len(base_indent) and not _is_toplevel_fence(i):
                block_inner: List[str] = []
                while i < n:
                    cur = lines[i]
                    block_inner.append(cur)
                    i += 1
                    if fence_close_re.match(cur):
                        break
                stripped = [re.sub(rf'^{re.escape(base_indent)}', '', b, count=1) for b in block_inner]
                reindented = [cont_indent + b for b in stripped]
                out.extend(reindented)
                continue
            out.append(nxt)
...
def _inside_admonition_div(node) -> bool:
    """Return True if 'node' is inside a <div class="note/warning/important/...">."""
    ADMON_CLASSES = {"note", "warning", "important", "information", "tip", "danger"}
    p = node.parent if node else None
    while isinstance(p, Tag):
        if p.name == "div":
            classes = set(p.get("class") or [])
            if ADMON_CLASSES & classes:
                return True
        p = p.parent
    return False
    ...
    for lst in list(container.find_all(["ul", "ol"])):
        ...
        for li in list(lst.find_all("li")):
            ...
            for pnode in list(li.find_all("p")):
                if _in_itemgroup_info(pnode):
                    continue
                # Do not unwrap <p> inside admonition divs; markdownify needs
                # them to produce paragraph breaks (blank lines) in :::note blocks.
                if _inside_admonition_div(pnode):
                    continue
                if not pnode.get_text(strip=True):
                    pnode.decompose()
                else:
...
def get_doc_container_with_fallback(url: str, session: requests.Session) -> tuple[Optional[Tag], BeautifulSoup]:
    ...
    soup = BeautifulSoup(fix_mojibake_text(resp.text), "html.parser")
    ...
    if _match_docs_host(url):
        ...
        if rendered_html:
            soup_pw = BeautifulSoup(fix_mojibake_text(rendered_html), "html.parser")
...
def normalize_url_for_crawl_match(u: str) -> str:
    """Normalize a docs URL for case-insensitive crawl matching and dedupe."""
    if not u:
        return u
    parsed = urllib.parse.urlparse(canonicalize_url(u))
    path = parsed.path or "/"
    if path != "/":
        path = path.rstrip("/")
...
def html_to_markdown_github(url, position_counter=None, crawl=True):
    ...
    try:
        ...
        normalized_base_url = normalize_url_for_crawl_match(base_url)
        ...
        visited_urls = set()
        queued_urls = {normalize_url_for_crawl_match(url)}
        queue = [url]
        ...
        while queue:
            ...
            normalized_current_url = normalize_url_for_crawl_match(current_url)
            queued_urls.discard(normalized_current_url)
            ...
            try:
                ...
                if not doc_container or not doc_container.get_text(strip=True):
                    ...
                    # Strip TOPLEVEL_NOTE markers (used by indent functions)
                    markdown_content = re.sub(r'(^|(?<=\S) )TOPLEVELNOTEMARKER\s*(?=\n|$)', r'\1', markdown_content, flags=re.M)
                ...
                if title_tag and title_tag.string and title_tag.string.strip():
                    title = fix_mojibake_text(clean_title(title_tag.string.strip()))
                else:
                    title = fix_mojibake_text(slug.replace("-", " ").strip().title())
                ...
                full_markdown_content = fix_mojibake_text(full_markdown_content)
                ...
                if crawl:
                    for link in soup.find_all("a", href=True):
                        ...
                        normalized_absolute_url = normalize_url_for_crawl_match(absolute_url)
                        
                        # Only crawl within this guide's base_url scope
                        if (normalized_absolute_url.startswith(normalized_base_url)
                                and normalized_absolute_url not in visited_urls
                                and normalized_absolute_url not in queued_urls):
...
def derive_metadata_from_url(url: str) -> Dict[str, object]:
    segs = _segments(url)
    product_slug = segs[0].lower() if len(segs) >= 1 else ""
...
def href_from_li(li: Tag, base_url: str) -> Optional[str]:
    ...
    # Active/current page: sidebar renders <li aria-current="true"> with a <span> instead of <a>.
    # Derive path directly from base_url to preserve the correct URL slug.
    if str(li.get("aria-current", "")).lower() == "true":
