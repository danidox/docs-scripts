...
def rename_acloud_files(
    ...
    """
    For every -acloud file in the mapping, rename it on disk to strip the -acloud suffix.
    e.g. overview-acloud.md -> overview.md
         _overview-acloud.mdx -> _overview.mdx

    In dry run mode, just prints what would happen.
    """
    print("\n--- Renaming -acloud files ---")
    for base, mp in sorted(mapping.items()):
        if ACLOUD not in mp.variants:
            continue

        filename = mp.variants[ACLOUD]  # e.g. "overview-acloud.md" or "_overview-acloud.md"
        # Find the actual file on disk by scanning content_dir
        ...
        for old_path in matches:
            ext = old_path.suffix
            stem = old_path.stem  # e.g. "overview-acloud" or "_overview-acloud"
            new_stem = re.sub(rf"-{ACLOUD}$", "", stem, flags=re.IGNORECASE)
            new_path = old_path.with_name(new_stem + ext)

            if old_path == new_path:
                print(f"  SKIP: {old_path.name} (already has no -acloud suffix)")
                continue

            if apply:
                if new_path.exists():
                    print(f"  SKIP: {old_path.name} -> {new_path.name} (target already exists)")
                    log_handle.write(f"  SKIP rename {old_path} -> {new_path} (target exists)\n")
                    continue
                old_path.rename(new_path)
                print(f"  RENAMED: {old_path.name} -> {new_path.name}")
                log_handle.write(f"  Renamed: {old_path} -> {new_path}\n")
            else:
                print(f"  DRY RUN: would rename {old_path.name} -> {new_path.name}")
                log_handle.write(f"  Would rename: {old_path} -> {new_path}\n")
    ...
    with open(LOG_FILE, "a", encoding="utf-8") as log_handle:
        ...
        while True:
            ...
            if user_input == "":
                ...
                rename_acloud_files(mapping, content_dir, apply=args.apply, log_handle=log_handle)
                break
