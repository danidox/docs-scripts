...
To open the Project explorer, select ![](/images/studio-web/studio-web-image-project-explorer.png) on the upper-left side of the designer. This is where you can:

* Rename the project by right-clicking it and selecting **Rename**.
* Open a workflow in the designer by selecting it.
* Add an activity, a workflow, or a folder using the **Add to project** button on the upper-right side of the panel. File and folder names can't start with the character `.` or contain the following special characters: `' ", < > | : * ? \ / ; % =`.
* Search for files and folders using the **Search** button on the upper-right side of the panel.
...
* Import individual project files and resources by right-clicking the project and selecting **Import item into project**. The project files can belong to standalone automation projects or to projects that are part of a solution. Selecting this option opens the **Import item** window, which contains a list of available projects. Expand a project from the list, choose the project file or resource that you want to import, and select **Add** to include it in your current project.
  :::note
  Importing creates a copy of the imported file in the current project.
  :::
