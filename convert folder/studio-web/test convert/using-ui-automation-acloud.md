...
* **Design time** - You can design automations with UI Automation activities in Studio Web from:
  + **Google Chrome** - available on Windows and macOS.
  + **Microsoft Edge** - available on Windows.
  + **Safari** - available on macOS.Studio Web and the target web apps must be rendered in the same browser (Chrome, Edge, or Safari) and the UiPath extension must be [installed for the browser](https://docs.uipath.com/studio-web/automation-cloud/latest/everyone-user-guide/using-ui-automation#installing-the-uipath-browser-extension).
* **Run time** - Automations built with UI Automation and Studio Web run by default on serverless robots in the UiPath Cloud in Chrome. With the help of UiPath Orchestrator, they can be configured to run unattended on Windows, macOS, and Automation Cloud robots where the browser and the corresponding UiPath extension are installed.
  :::note
  Support for the Safari browser extension is only available if you install UiPath Assistant 2024.10.5 or higher.
  :::

:::note
In response to [Google’s timeline for Chrome Manifest V2 (MV2) deprecation](https://developer.chrome.com/docs/extensions/develop/migrate/mv2-deprecation-timeline), starting from June 2024, existing Studio Web extensions for Chrome and Edge have been updated to be fully compatible with Manifest V3, while retaining the same extension IDs for both browsers ([Chrome](https://chrome.google.com/webstore/detail/uipath-extension-for-stud/conkfbpnllelocpogdmbilgmnkabjfmf) and [Edge](https://microsoftedge.microsoft.com/addons/detail/uipath-extension-for-stud/hmpkmollbbcoopahpbplbdaapacncbbc)). No action is required to receive the new MV3 extensions, which will be automatically updated when you reopen the browser. Following the upgrade to MV3, [the debugging banner notification](https://docs.uipath.com/studio-web/automation-cloud/latest/everyone-user-guide/debugging-banner-notification-after-extension-upgrade-to-manifest-v3#debugging-banner-notification-after-extension-upgrade-to-manifest-v3) will be displayed each time you start an interactive selection. When the automation runs on serverless robots, the MV2 extension is still used. The robot serverless image will also be updated to include the pre-installed MV3 extension for Chrome before the [Chrome MV2 deprecation in stable versions](https://developer.chrome.com/docs/extensions/develop/migrate/mv2-deprecation-timeline#june_2024_1-x_months_chrome_mv2_deprecation_stable_rollout).
:::
...
4. Select **Install extension**. A new browser tab opens prompting you to install the extension you need.
   !['Install extension' image](https://docs-dev.uipath.com/api/binary/studio-web/2/627891/595840)
