...
* **Panel view** - All the properties of the selected activity, including advanced properties, appear only in the Properties panel and the activity card is collapsed.
  ![docs image](https://docs-dev.uipath.com/api/binary/studio-web/2/627891/404384)

Selecting **See more** ![docs image](https://documentationpicturerepo.blob.core.windows.net/screenshots/screenshots/StudioWeb/see-more.png) next to an activity field displays the options for that field. The options for most fields are:
...
4. In the **Set value** field, select **See more** ![docs image](https://documentationpicturerepo.blob.core.windows.net/migrated/More_VT.png) &gt; **Open expression editor**.
...
8. In the **To variable** output field, select **MyVariable**.
   ![docs image](https://docs-dev.uipath.com/api/binary/studio-web/2/627891/381843)
...
11. Select **Debug on cloud** at the top of the designer to test the automation, and check the run output panel to see the variable changed from lowercase ("example") to uppercase ("EXAMPLE").
    ![docs image](https://docs-dev.uipath.com/api/binary/studio-web/2/627891/381860)
...
3. Select **Any (OR)** from the dropdown at the top of the window to indicate that the creation of files with any of the extensions should trigger the automation.
   ![docs image](https://docs-dev.uipath.com/api/binary/studio-web/2/627891/276038)
...
3. In the Condition Builder, add the condition "the file size is less than 1MB". We will use the **SizeInBytes** property of the created file, and 1MB = 1000000 Bytes.
   ...
   ![docs image](https://docs-dev.uipath.com/api/binary/studio-web/2/627891/359828)
4. We will then add two **Move File** activities to move the created file as follows:
   ...
   * The other activity in the **Else** branch of the If activity. This is the activity executed when the condition is not met (the file size is not under 1MB) so we will select **Big files** as the destination folder.
   ![docs image](https://docs-dev.uipath.com/api/binary/studio-web/2/627891/386172)
...
1. In the Upload Files activity, select **See more** ![docs image](https://documentationpicturerepo.blob.core.windows.net/screenshots/screenshots/StudioWeb/see-more.png) &gt; **Build a collection of files** next to the **File(s)** field.
...
3. Select **Add**, then click the first field and select the output of the first Download File activity. Repeat for the output of the second activity, and then click **Save**.
   ![docs image](https://docs-dev.uipath.com/api/binary/studio-web/2/627891/359844)
...
Select a date from the calendar and a time of day (hour and minute). Available for fields that require a date and time value.
   ![docs image](https://docs-dev.uipath.com/api/binary/studio-web/2/627891/275952)
...
3. Select the activity field you want to edit. The Data Mapping editor opens.
   ![docs image](https://docs-dev.uipath.com/api/binary/studio-web/2/627891/569848)

Alternatively, you can reach the Data Mapping editor by selecting **See More** ![](https://docs-dev.uipath.com/api/binary/studio-web/2/627891/549570) &gt; **Data Mapping** next to the activity field.
...
* Map an object property to an activity output in the **Mapping** column. Select the field next to each property and choose a variable from the **Use variables** selector. You can also select the **See more** ![docs image](https://docs-dev.uipath.com/api/binary/studio-web/2/627891/549570) button to:
