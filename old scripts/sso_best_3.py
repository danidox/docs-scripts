...
def unwrap_markdown_paragraphs_and_lists(md_text: str) -> str:
    """
    Unwrap soft line breaks inside Markdown paragraphs AND list items,
    while preserving code blocks, headings, and overall structure.
    ...
    Example:
      ...
      Output:
        - Sticky keys are also supported. For example, if you want to type `Shift`+`End`
          to select only part of the text of a control, you need to select `Shift` and `End`
          from the list of special keys...
    """
    ...
    def flush_buffer():
        if not buffer:
            return
        # Detect list indentation level (keep indent)
        indent_match = list_prefix.match(buffer[0])
        indent = indent_match.group(1) if indent_match else ""
        # Join all wrapped lines into one
    ...
    for line in lines:
        ...
        # Headings or admonition markers
        if stripped.startswith("#") or stripped.startswith(":::"):
            flush_buffer()
            new_lines.append(line)
            continue

        # List item or continuation of list
        if list_prefix.match(line):
            flush_buffer()
            buffer.append(line)
            continue

        # Continuation of list or paragraph — same indent or no indent
        if buffer:
            # Merge with previous if likely continuation
            buffer.append(line)
        else:
            buffer.append(line)
