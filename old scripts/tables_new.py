...
- Additionally: collapse inner content of each <td>...</td> to a single line,
  normalizing whitespace (newlines and multiple spaces → single space) but preserving
  all HTML tags (e.g. <br>, <em>, etc.).
...
# --- NEW: collapse <td> content to a single line ---
TD_ONELINE_RE = re.compile(r'(<td\b[^>]*>)(.*?)(</td>)', re.IGNORECASE | re.DOTALL)
...
# --- NEW: helper to collapse <td> inner content onto a single line ---
def collapse_td_content(match: re.Match) -> str:
    opening, inner, closing = match.groups()
    # Normalize whitespace: any run of whitespace (spaces, tabs, newlines) → single space
...
def rewrite_table_block(table_html: str) -> str:
    ...
    # --- NEW: collapse each <td>...</td> onto a single line ---
    html = TD_ONELINE_RE.sub(collapse_td_content, html)
