...
def transform_admonitions(container: BeautifulSoup) -> None:
    ...
    for div in container.find_all("div", class_=True):
        ...
        # Build single-line fence (no newlines yet). Collapse inner newlines/whitespace to spaces.
        inner_inline = re.sub(r"\s+", " ", inner_md.strip())
        block = f":::{admonition} {inner_inline} :::"

        # Keep as-is (no leading newline); list alignment handled later after expansion.
...
def unwrap_markdown_paragraphs_and_lists(md_text: str) -> str:
    ...
    table_line = re.compile(r'^\s*\|')  # pipe table rows
    table_divider = re.compile(r'^\s*[:\-| ]+$')  # header divider like |---| or :-: etc.
    ...
    def flush_buffer():
        ...
        if m:
            indent = m.group(1)
        merged = " ".join(line.strip() for line in buffer).strip()
    ...
    for line in lines:
        ...
        # Blank lines end paragraphs and tables
        if not stripped:
            flush_buffer()
            inside_table = False
            new_lines.append("")
        ...
        # List item starts a fresh buffer
        ...
        # Continuation of current paragraph/list
        buffer.append(line)
...
def indent_images_like_list_content(md_text: str) -> str:
    ...
    while i < n:
        ...
        while i < n:
            ...
            # Keep blank lines (do NOT collapse before images)
            if not nxt.strip():
                out.append(nxt)
                i += 1
                continue
...
def expand_single_line_admonitions(md_text: str) -> str:
    """
    Turn inline admonitions like ':::type content :::' into 3-line fences:
        :::type
        content
        :::
    Inserts a newline after {admonition} and one after {inner_md}.
    Preserves leading indent so list alignment works.
    """
    import re
    out = []
    single_line = re.compile(r'^(?P<indent>\s*):::([A-Za-z]+)\s+(?P<content>.*?)\s+:::\s*$')
    for line in md_text.splitlines():
        ...
        admo_type = re.match(r'^\s*:::([A-Za-z]+)', line).group(1)
        content = m.group("content").strip()
        out.append(f"{indent}:::{admo_type}")
...
def indent_notes_like_list_content(md_text: str) -> str:
    """
    Inside list items, indent Docusaurus fences (:::note/tip/warning/etc.)
    to the continuation indent (same column as list content), and also
    reindent the first following paragraph line to that same indent.
    Collapses a single blank line before/after the fence when appropriate.
    """
    import re
    ...
    while i < n:
        ...
        # at a list-item head → compute continuation indent
        base_indent, marker, _ = m.groups()
        cont_indent = base_indent + (" " * (len(marker) + 1))

        out.append(line); i += 1

        # optional single blank line before fence
        if i < n and not lines[i].strip():
            if i + 1 < n and fence_open_re.match(lines[i + 1]) and lines[i + 1].startswith(base_indent):
                i += 1  # drop the blank line

        # if a fence opens at base indent, reindent whole fence block
        if i < n and fence_open_re.match(lines[i]) and lines[i].startswith(base_indent):
            ...
            while i < n:
                ...
                if fence_close_re.match(cur):
                    break
            # base → continuation indent for every fence line
            block = [re.sub(rf'^{re.escape(base_indent)}', cont_indent, b, count=1) for b in block]
            out.extend(block)

            # optional single blank line after fence (drop only if we’re continuing same list item)
            if i < n and not lines[i].strip():
                if i + 1 < n and not re.match(rf'^{re.escape(base_indent)}([-*+]|\d+\.)\s+', lines[i + 1]):
                    i += 1

            # reindent the first following paragraph line if it’s still this list item
            if i < n:
                nxt = lines[i]
                if nxt.strip() and nxt.startswith(base_indent) \
                   and not re.match(rf'^{re.escape(base_indent)}([-*+]|\d+\.)\s+', nxt) \
                   and not code_fence_re.match(nxt) \
                   and not fence_open_re.match(nxt):
                    lines[i] = re.sub(rf'^{re.escape(base_indent)}', cont_indent, nxt, count=1)

            continue  # done with this list item’s fence handling

        # otherwise, just copy lines until a boundary
        while i < n:
            nxt = lines[i]
            if re.match(rf'^{re.escape(base_indent)}([-*+]|\d+\.)\s+', nxt) or code_fence_re.match(nxt):
                break
            out.append(nxt); i += 1

    return "\n".join(out)

def ensure_blank_lines_around_blocks(md_text: str) -> str:
    """
    Ensure there is exactly one blank line before and after:
      - Docusaurus admonition fences (:::note, :::tip, :::warning, :::)
      - Markdown image blocks (![...](...))
      - Code fences (``` / ~~~)
    Preserves indentation if the block is inside a list item.
    """
    import re

    # Force newline before/after fences, images, code blocks, respecting indent.
    def add_blank_lines(text: str, pattern: str) -> str:
        """
        For each match of `pattern`, ensure:
          - one blank line before (unless at start)
          - one blank line after (unless immediately followed by same indent blank)
        """
        ...
        while i < n:
            ...
            if block_re.match(line):
                ...
                # Ensure blank line before this block (if previous non-blank is not same indent)
                if len(out) >= 2 and out[-2].strip() and not out[-2].startswith(indent):
                    out.insert(-1, "")

                # Skip through whole fenced/code block if multiline
                ...
                if i + 1 < n and lines[i + 1].strip() and not lines[i + 1].startswith(indent):
                    out.append("")
    ...
    # 1. Admonition fences
    md_text = add_blank_lines(md_text, r'^\s*:::\w*')
    # 2. Code fences
    md_text = add_blank_lines(md_text, r'^\s*(```|~~~)')
    # 3. Image blocks
    md_text = add_blank_lines(md_text, r'^\s*!\[.*\]\(.*\)')
    # Ensure a blank line after closing ::: fences (preserving indent)
    md_text = re.sub(
        r'(?m)^(\s*:::\s*)$',
        r'\1\n',
        md_text
        ...
        while queue:
            ...
            try:
                ...
                if doc_container:
                    ...
                    markdown_content = expand_single_line_admonitions(markdown_content)
                    markdown_content = indent_notes_like_list_content(markdown_content)
                    markdown_content = ensure_blank_lines_around_blocks(markdown_content)
