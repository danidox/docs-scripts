...
_BARE_URL = re.compile(
    ...
    r'(?<!href=")'               # not inside an HTML href="
    r'(?P<url>https?://[^\s<>\])}"]+?)'  # the URL
    r'(?P<trail>[)\]\}.,!?;:]*)(?=\s|$)', # capture trailing punctuation to keep outside link
    flags=re.IGNORECASE
)
...
def convert_download_links_to_markdown(container: BeautifulSoup) -> None:
    """
    Convert <div><p class="p"><a href=...><span>Text</span></a></p></div> style links
    into pure Markdown link syntax.
    Example:
        <div><p class="p"><a href="url"><span>Label</span></a></p></div>
    becomes:
        [Label](url)
    """
    for div in list(container.find_all("div")):
        ...
        if a_tag and a_tag.find("span"):
            ...
            div.replace_with(NavigableString(md_link))
...
_FENCE_START = re.compile(r'^[ \t]*(?P<fence>`{3,}|~{3,})(?P<info>[^\n`]*)?$')
_FENCE_END   = lambda token: re.compile(r'^[ \t]*' + re.escape(token) + r'[ \t]*$')
_INDENTED    = re.compile(r'^(?: {4}|\t)')
_LINK_MD_RE  = re.compile(
    r'\[(?P<text>[^\]]+)\]\((?P<url>https?://[^)\s]+)(?P<title>\s+"[^"]*")?\)'
)

def _split_fenced_and_indented(md: str):
    out, buf = [], []
    in_fence, fence_token, in_indent = False, None, False
    def flush(is_code):
        if buf: out.append(("".join(buf), is_code)); buf.clear()
    lines = md.splitlines(keepends=True)
    for i, line in enumerate(lines):
        if in_fence:
            buf.append(line)
            if _FENCE_END(fence_token).match(line.strip()):
                flush(True); in_fence, fence_token = False, None
            continue
        m = _FENCE_START.match(line)
        if m:
            flush(False); in_fence, fence_token = True, m.group('fence')
            buf.append(line); continue
        if _INDENTED.match(line):
            if not in_indent: flush(False); in_indent = True
            buf.append(line)
            if i+1 < len(lines) and not _INDENTED.match(lines[i+1]):
                flush(True); in_indent = False
            continue
        buf.append(line)
    flush(in_fence or in_indent)
    return out

def _split_inline_backticks(s: str):
    parts, i, n = [], 0, len(s)
    while i < n:
        if s[i] != "`":
            j = s.find("`", i)
            if j == -1: parts.append((s[i:], False)); break
            parts.append((s[i:j], False)); i = j; continue
        run = 1; i += 1
        while i < n and s[i] == "`": run += 1; i += 1
        close = s.find("`"*run, i)
        if close == -1: parts.append((("`"*run)+s[i:], False)); break
        parts.append((s[i-run:close+run], True)); i = close + run
    return parts

def unescape_underscores_in_markdown_links(md_text: str) -> str:
    """Unescape \\_ in Markdown link URLs; also unescape in link text if it looks like a URL.
    Skips fenced and inline code."""
    def fix_segment(seg: str) -> str:
        def repl(m):
            text  = m.group('text')
            url   = m.group('url').replace(r'\_', '_')
            title = m.group('title') or ''
            # If the visible text itself looks like a URL, unescape there too
            if text.startswith('http://') or text.startswith('https://'):
                text = text.replace(r'\_', '_')
            return f'[{text}]({url}{title})'
        return _LINK_MD_RE.sub(repl, seg)

    out = []
    for block, is_code in _split_fenced_and_indented(md_text):
        if is_code:
            out.append(block); continue
        rebuilt = []
        for chunk, is_inline_code in _split_inline_backticks(block):
            rebuilt.append(chunk if is_inline_code else fix_segment(chunk))
        out.append("".join(rebuilt))
    return "".join(out)


def escape_underscores_in_plain_text_markdown(md_text: str) -> str:
    """
    Escape '_' in plain Markdown text, but NOT inside:
      - fenced/indented code blocks
      - inline code (backticks)
      - Markdown links [text](url)
    """
    def escape_outside_links(seg: str) -> str:
        # escape underscores that are not already escaped
        def esc_unders(s: str) -> str:
            return re.sub(r'(?<!\\)_', r'\\_', s)

        out, pos = [], 0
        for m in _LINK_MD_RE.finditer(seg):
            out.append(esc_unders(seg[pos:m.start()]))  # plain text before link
            out.append(m.group(0))                      # keep the whole link as-is
            pos = m.end()
        out.append(esc_unders(seg[pos:]))               # tail
        return "".join(out)

    pieces = []
    for block, is_code in _split_fenced_and_indented(md_text):
        if is_code:
            pieces.append(block)  # keep fenced/indented code blocks untouched
            continue
        rebuilt = []
        for chunk, is_inline_code in _split_inline_backticks(block):
            rebuilt.append(chunk if is_inline_code else escape_outside_links(chunk))
...
from bs4 import BeautifulSoup, NavigableString, Tag  # already present

def _absorb_following_into_note_if_empty(div: Tag) -> None:
    """
    If a <div class="note"> is empty (e.g., only had a 'Note:' label removed),
    pull adjacent text/inline blocks that follow it back inside.
    Stops at clear block boundaries (another note, list/table/pre, etc.).
    """
    def is_inlineish(tag: Tag) -> bool:
        return tag.name in {"span", "a", "strong", "em", "code", "kbd", "sup", "sub", "b", "i"}

    # Note already has content? Do nothing.
    if div.get_text(strip=True):
        return

    for sib in list(div.next_siblings):
        # Stop at boundaries that should not be absorbed
        ...
        if isinstance(sib, NavigableString):
            if str(sib).strip():
                # wrap text in a <p> to keep markdownify clean
                p = div.new_tag("p")
                p.string = str(sib)
                div.append(p)
            sib.extract()
...
def transform_admonitions(container: BeautifulSoup) -> None:
    ...
    ADMONITION_KEYS = set(CLASS_TO_ADMONITION.keys())

    for div in container.find_all("div", class_=True):
        classes = set(div.get("class", []))
        # 🔍 accept any admonition-like class, not just 'note'
        if not (ADMONITION_KEYS & classes):
            continue

        # choose the most specific class present
        admonition = "note"
        for key in ("danger","warning","attention","caution","important","tip","info","information","note"):
            if key in classes:
                admonition = CLASS_TO_ADMONITION[key]
                break

        # NEW: if empty, pull trailing content inside first
        ...
        in_list = div.find_parent("li") is not None
        if in_list:
            indent = "    "
            block = "\n".join((indent + line) if line else indent for line in block.splitlines())
...
def backslash_escape_mdx_specials(container) -> None:
    """
    Make MDX-safe without introducing visible backslashes:
    - Do NOT escape underscores at all (CommonMark/MDX handle identifiers like My_var_name fine).
    - Replace <, >, {, } in normal text with HTML entities (&lt;, &gt;, &#123;, &#125;).
    - Skip HTML <code>/<pre> nodes.
    - Skip Markdown code spans delimited by backticks (`...` or ```...```).
    - Keep '*' intact.
    - Leave {{...}} placeholders untouched.
    - Do not touch text inside links for underscores (not needed anymore).
    """
    if not container:
        return

    skip_tags = {"code", "pre"}
    PLACEHOLDER_INLINE_RE = re.compile(r"{{[^}]*}}")
    ...
    def split_markdown_code_segments(s: str):
        """
        Split into [(segment, is_code), ...] using backtick runs.
        Protects inline (`...`) and fenced (```...```) segments that both start
        and end within this text node.
        """
        parts = []
        i, n = 0, len(s)
        while i < n:
            ...
            if close == -1:
                ...
                parts.append((("`" * run) + s[i:], False))
                break
            end = close + run
            parts.append((s[close - run:end], True))  # include delimiters
            # Prepend the non-code between last non-code and this opener
            # (already added before we hit the opening backticks)
            # Move on
            i = end
        return parts

    def html_entity_escape(chunk: str) -> str:
        # Avoid double-escaping if already &lt; or &gt; etc.
        # Do minimal replacements that matter for MDX/JSX parsing.
        # Note: we don't touch underscores at all.
        # First, protect existing entities to avoid mangling them.
        chunk = chunk.replace("&lt;", "\uFFFFLT;").replace("&gt;", "\uFFFFGT;")
        chunk = chunk.replace("&#123;", "\uFFFFLBRACE;").replace("&#125;", "\uFFFFRBRACE;")

        chunk = chunk.replace("<", "&lt;").replace(">", "&gt;")
        chunk = chunk.replace("{", "&#123;").replace("}", "&#125;")

        # Restore previously-protected entities
        chunk = chunk.replace("\uFFFFLT;", "&lt;").replace("\uFFFFGT;", "&gt;")
        chunk = chunk.replace("\uFFFFLBRACE;", "&#123;").replace("\uFFFFRBRACE;", "&#125;")
        return chunk
    ...
    for tn in list(container.find_all(string=True)):
        if not isinstance(tn, NavigableString):
            continue
        s = str(tn)
        if not s or in_tags(tn, skip_tags) or PLACEHOLDER_INLINE_RE.search(s):
            continue

        # Split around markdown code spans; escape only non-code chunks
        chunks = split_markdown_code_segments(s)
        # Fallback if splitter produced nothing (shouldn't happen)
        if not chunks:
            chunks = [(s, False)]

        out = []
        for seg, is_code in chunks:
            if is_code:
                out.append(seg)  # leave code span (with backticks) untouched
            else:
                out.append(html_entity_escape(seg))
        ...
        for li in list(lst.find_all("li")):
            ...
            for wrapper_name in ("div", "span"):
                for w in list(li.find_all(wrapper_name)):
                    ...
                    # 🚫 Do NOT unwrap admonition containers (note, warning, tip, etc.)
                    classes = set(w.get("class") or [])
                    if {"note","information","info","tip","important","warning","caution","attention","danger"} & classes:
                        continue
                    # also keep common UiPath wrappers like AdmonitionContainer by id
...
def unwrap_wrapped_list_text(md_text: str) -> str:
    """
    Reflow soft-wrapped list item text into a single line.
    - Works for ordered and unordered lists.
    - Preserves nested lists and stops at new list items, blank lines,
      code fences, tables, blockquotes, etc.
    """
    lines = md_text.splitlines()
    out = []
    i, n = 0, len(lines)
    ...
    while i < n:
        ...
        indent, bullet, num, content = m.groups()
        marker = (bullet + " ") if bullet else (f"{num}. ")
        ...
        while j < n:
            ...
            # stop if blank or code-fence or a new list item at same/higher level
            if not nxt.strip():
                break
            if _FENCE_START.match(nxt.strip()):           # from existing code
                break
            if _LIST_START_RE.match(nxt):                 # new peer list item
                break
            if nxt.lstrip().startswith("|"):              # table row
                break
            if nxt.lstrip().startswith(">"):              # blockquote
                break
            #if nxt.lstrip().startswith(":::"):            # admonitions
                #break

            # treat as continuation only if it's an inline wrap (same indent + 2 spaces)
            # and not itself starting a (more-indented) sub list
            cont_indent = indent + "  "
            if nxt.startswith(cont_indent) and not _LIST_START_RE.match(nxt[len(indent):]):
                content += " " + nxt.strip()
                j += 1
                continue
            break
        ...
        while queue:
            ...
            try:
                ...
                if doc_container:
                    ...
                    convert_download_links_to_markdown(doc_container)
                    ...
                    # 1. Unwrap <code> only if it's directly inside <pre>
                    for pre in doc_container.find_all("pre"):
                        for code in pre.find_all("code", recursive=False):
                            code.unwrap()

                    # 2. Remove redundant clipboard-copy spans
                    ...
                    markdown_content = md.markdownify(str(doc_container), heading_style="ATX", strip=["br"])  # ignore any surviving <br>
                    markdown_content = cleanup_markdown(markdown_content)
                    markdown_content = unescape_underscores_in_markdown_links(markdown_content)
                    markdown_content = escape_underscores_in_plain_text_markdown(markdown_content)
                    ...
                    # 👇 NEW: unwrap soft-wrapped list item text
                    markdown_content = unwrap_wrapped_list_text(markdown_content)
