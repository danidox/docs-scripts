...
# Track whether we've already prompted for docs-dev/docs-staging login in this run
...
# ==============================
# === docs-staging SSO helpers  ====

DOCS_DEV_HOSTS = {"docs-dev.uipath.com", "docs-staging.uipath.com", "preview-docs.uipath.com"}

def _match_docs_host(url: str) -> str | None:
    """Return the matching docs host (docs-dev/docs-staging/preview) for a URL, or None."""
    try:
        netloc = urllib.parse.urlparse(url).netloc.lower()
    except Exception:
        return None
    for h in DOCS_DEV_HOSTS:
...
def _detect_kid_from_cloudflare(target_url: str, session: requests.Session) -> str | None:
    ...
    Works for any host in DOCS_DEV_HOSTS (docs-dev, docs-staging, preview, etc.).
    """
    host = _match_docs_host(target_url) or "docs-dev.uipath.com"

    try:
        # Build app-host login first, using the right host
    ...
    # Try team endpoint without kid (still host-agnostic, base stays CLOUDFLARE_LOGIN_BASE)
    try:
...
def _build_cf_login_url(redirect_path: str) -> str:
    """
    Build Cloudflare Access login URL(s) for docs-dev/docs-staging with optional KID.
    """
    kid = os.getenv("CLOUDFLARE_KID")

    if redirect_path.startswith("http"):
        target = redirect_path
        host = _match_docs_host(redirect_path) or "docs-dev.uipath.com"
    else:
        host = "docs-dev.uipath.com"
        target = "https://" + host + (redirect_path if redirect_path.startswith("/") else ("/" + redirect_path))
...
def _try_playwright_login(target_url: str, session: requests.Session) -> bool:
    ...
    into the provided requests session. Returns True if cookies for docs-staging were
    ...
    try:
        with sync_playwright() as p:
            ...
            # Collect cookies and filter to docs.uipath.com
            ...
            if got_any:
                ...
                print("Didn't detect docs cookies from Playwright session.")
            return got_any
    except Exception as e:
...
def prompt_for_docs_dev_login(target_url: str, session: requests.Session):
    ...
    """
    Send the user to Cloudflare Access login for docs, ask them to choose Azure AD,
    ...
    host = _match_docs_host(target_url) or parsed.netloc
    ...
    print("Please copy cookies from your browser's DevTools (Application → Cookies → docs.uipath.com).")
    ...
    if cookie_str.strip():
        ...
        print("2) You'll be redirected back to the docs page.")
...
def _is_docs_dev_url(url: str) -> bool:
    return _match_docs_host(url) is not None


def configure_docs_dev_auth(session: requests.Session, host: str | None = None):
    """
...
def get_content_container(soup: BeautifulSoup) -> Optional[Tag]:
    ...
    2) <main> tag
    3) divs with common doc-ish IDs/classes
    ...
    # 2) many modern layouts put the article inside <main>
    main = soup.find("main")
    if main:
        return main

    # 3) a couple of reasonable fallbacks (extend as needed)
    candidates = [
    ...
    # 4) absolute last resort: chew on the whole body
    if soup.body:
        return soup.body
...
def get_with_retries(
    ...
    **request_kwargs,
    ...
    # Allow callers to override headers; fall back to default UA if none provided
...
def sso_get_with_retries(url, session=None, max_tries=5, base_sleep=1.0, timeout=20, **request_kwargs):
    """
...
def html_to_markdown_github(url, position_counter=None, crawl=True):
    ...
    def get_content_container(soup: BeautifulSoup) -> Optional[Tag]:
        ...
        # 2) generic modern wrapper
        main = soup.find("main")
        if main:
            return main

        # 3) other likely wrappers (extend as needed for your layout)
        candidates = [
            soup.find("div", id="docs-container"),
            soup.find("div", id="content"),
            soup.find("div", class_="DocContainer"),
    ...
    try:
        ...
        while queue:
            ...
            try:
                ...
                # NEW: robust content container selection
                doc_container = get_content_container(soup)
                if not doc_container:
                    print(f"⚠️  No usable content container found (DocContainer/main/body): {current_url}")
                    continue
                ...
                # remove <...> around bare http/https links
