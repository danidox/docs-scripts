 ...
 * ────────────────────────────────────────────────────────────────────────────
 * Compares a guide between dev (docs-dev.uipath.com) and prod (docs.uipath.com).
...
const GUIDE_PATH  = getArg('--path') || '/apps/automation-cloud/latest/user-guide/introduction';
const OUTPUT_FILE = getArg('--output') || null;
const PAGE_LIMIT  = getArg('--limit') ? parseInt(getArg('--limit'), 10) : Infinity;
...
const DEV_BASE     = 'https://docs-dev.uipath.com';
const PROD_BASE    = 'https://docs.uipath.com';
  ...
  try {
    ...
    const valid = data.cookies && data.cookies.some(
      c => (c.name === 'CF_AppSession' || c.name === 'CF_Authorization')
           && c.domain.includes('docs-dev.uipath.com')
    );
...
async function navigateWithAuth(page, url, context, isDevSite, isHeadless) {
  ...
  try {
    ...
    await page.waitForFunction(
      () => window.location.hostname === 'docs-dev.uipath.com' &&
            !document.body.textContent.includes('Sign in with') &&
            !document.title.toLowerCase().includes('access'),
      { timeout: 180_000 }
    );
 ...
 *   - headings:  [{level, text}]           — all h1–h3 in document order
...
async function extractPageData(page) {
  return page.evaluate(() => {
    ...
    const candidates = ['article', 'main article', '[class*="docItem"]', 'main'];
    let el = null;
    ...
    clone.querySelectorAll('li').forEach(li => {
      ...
      if (text) {
        ...
        listItems.push({ depth, type, text });
      }
    });
    ...
    // Walk h1–h4, p, td, th in document order so each block is tagged with
    // the section heading it falls under — used to show WHERE missing content is.
    const paragraphs = [];
    let currentSection = null;
    clone.querySelectorAll('h1, h2, h3, h4, p, td, th').forEach(el => {
    ...
    clone.querySelectorAll('table').forEach(table => {
      ...
      const headerText = (firstCell || rows[0]).textContent.trim().replace(/\s+/g, ' ').slice(0, 80);
      const colCount = rows[0].querySelectorAll('td, th').length;
      if (headerText) tables.push({ headerText, rowCount: rows.length, colCount });
    });
...
async function crawlSite(context, seedUrl, isDevSite, isHeadless) {
  ...
  log(`  Seed: ${seedUrl}`);
  const status = await navigateWithAuth(page, seedUrl, context, isDevSite, isHeadless);
  if (status === 'reauth') { await page.close(); return null; }
  if (status === 'timeout') { await page.close(); return pages; }
  ...
  if (links.length === 0) {
    ...
    pages.set(normPath(seedUrl), { label: 'Introduction', url: seedUrl, ...(await extractPageData(page)) });
    await page.close();
    return pages;
  }

  const limited = links.slice(0, PAGE_LIMIT);
  if (PAGE_LIMIT < links.length) log(`  Limiting to first ${PAGE_LIMIT} of ${links.length} pages`);
  log(`  Crawling content for ${limited.length} pages…`);
  for (let i = 0; i < limited.length; i++) {
    const { label, pathname } = limited[i];
    const fullUrl = toUrl(base, pathname);
    process.stdout.write(`\r  [${i + 1}/${limited.length}] ${label.slice(0, 60).padEnd(60)}`);
    try {
      if (normPath(page.url()) !== pathname)
        await page.goto(fullUrl, { waitUntil: 'domcontentloaded', timeout: 30_000 });
      await page.waitForTimeout(400);
      pages.set(pathname, { label, url: fullUrl, ...(await extractPageData(page)) });
    } catch (e) {
      pages.set(pathname, { label, url: fullUrl, headings: [], text: '', wordCount: 0, error: e.message });
    }
  }
...
async function crawlSinglePage(context, pathname, isDevSite, isHeadless) {
  ...
  const fullUrl = toUrl(base, pathname);
  const page    = await context.newPage();
  const status  = await navigateWithAuth(page, fullUrl, context, isDevSite, isHeadless);
  if (status === 'reauth') { await page.close(); return null; }
  if (status === 'timeout') { await page.close(); return { label: pathname.split('/').pop(), url: fullUrl, headings: [], listItems: [], images: [], admonitions: [], paragraphs: [], tables: [], text: '', wordCount: 0 }; }
  await page.waitForTimeout(2000);
...
function compareListStructure(prodItems, devItems) {
  ...
  for (const prodItem of prodItems) {
    ...
    if (!devItem) {
      ...
      if (devItem.depth !== prodItem.depth && prodItem.depth > 1) {
        issues.push({
          kind: 'wrong-depth',
          text: prodItem.text,
...
function comparePage(prodPage, devPage) {
  ...
  const normH = (s) => s.toLowerCase().replace(/[^\p{L}\p{N} ]/gu, '').replace(/\s+/g, ' ').trim();
  ...
  const missingImages  = prodImageCount > devImageCount
    ? [{ count: prodImageCount - devImageCount, prodTotal: prodImageCount, devTotal: devImageCount }]
    : [];
  ...
  const normSnippet = (s) => s.toLowerCase().replace(/[^\p{L}\p{N} ]/gu, '').replace(/\s+/g, ' ').trim().slice(0, 60);
  ...
  const normPara = (s) => s.toLowerCase().replace(/[^a-z0-9]/g, ' ').replace(/\s+/g, ' ').trim();
  ...
  const normHeader = (s) => s.toLowerCase().replace(/[^\p{L}\p{N} ]/gu, '').replace(/\s+/g, ' ').trim().slice(0, 60);
  const devTableMap = new Map(
    (devPage.tables || []).map(t => [normHeader(t.headerText), t])
  );
  const tableIssues = [];
  for (const t of (prodPage.tables || [])) {
    const key  = normHeader(t.headerText);
    const devT = devTableMap.get(key);
    if (!devT) {
      tableIssues.push({ kind: 'missing-table', headerText: t.headerText, prodRows: t.rowCount, prodCols: t.colCount });
...
function printPageDetail(c, prodPage, devPage) {
  ...
  if (c.missingImages.length > 0) {
    ...
    section('IMAGES', mi.prodTotal, mi.devTotal);
    err(`  ✗ ${mi.count} image${mi.count > 1 ? 's' : ''} missing from dev`);
  }
...
function printReport(report, prodPages, devPages) {
  ...
  if (missingInDev.length === 0) {
    ...
    for (const p of missingInDev) {
      ...
      dim(`     Dev (missing): ${toUrl(DEV_BASE, p)}`);
...
async function main() {
  ...
  if (PAGE_SINGLE) {
    ...
    await browser.close();
    if (!devPage) { err('\nCould not retrieve dev page.'); process.exit(1); }
  ...
  if (!devPages || devPages.size === 0) {
    err('\nCould not retrieve dev pages. Check authentication and try again.');
    process.exit(1);
  }
